    ...
    for (int i = 0; i < current_bucket -> last_element; i++) {
        ...
        if (current_bucket -> data[i] -> pkt != 0x0) {
            ...
            */
            ...
            sent = rte_eth_tx_buffer(dst_port, 0, buffer, cloned_pkt);
            if (sent)
    ...
    *(uint16_t*)(udp + 2) = udp_tmp;
    ...
    if (nack_count != 0xff) {
        ...
        *first_seq_to_ack_ptr = ack_sequence_number+1;  // current seq number + 1 equal next seq number
        ...
        int del=1;
        if (efirstBit==1){
            if (array_contains(ack_pkt_id, nack_list, nack_count)){ // Skip the list of nacks    **** base of decision of being acked or not ****
                    del=0;
                    //continue;
            }
        }
        if (del==1){
            ...
            if(ret1 >= 0) {
                ...
                //RTE_LOG(INFO, L2FWD, "NOTE, ACK arrived DELETING FROM 0x%.4" PRIX16 " 0x%.16" PRIX16 "\n", first_seq_to_ack, ack_sequence_number);
...
main(int argc, char **argv)
{
    ...
    */
    ...
    clone_pktmbuf_pool = rte_pktmbuf_pool_create("CLONEPOOL", 4096, 32, 0, 0, rte_socket_id());
